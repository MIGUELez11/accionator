...
export function ScreenerType({ title, description, icon: Icon, selected, onClick }: ScreenerTypeProps) {
  return (
    <div
      className={cn(
      ...
      onClick={onClick}
      onKeyDown={handleOnKeyboardClick(onClick)}
      role="button"
      tabIndex={0}
    >
      <Icon className="w-8 h-8" />
      <p className="text-lg font-semibold text-wrap">{title}</p>
      <p className="text-sm text-secondary-foreground text-wrap">{description}</p>
    </div>
  );
}
